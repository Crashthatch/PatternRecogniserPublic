...
public class ReverseCSVToTableQuoteIfNeeded extends TransformationServiceReversible {

	public ReverseCSVToTableQuoteIfNeeded()
	{
	...
	public Table<Integer, Integer, String> doWork(Table<Integer, Integer, String> input) {
		...
		for (int rowIdx = 0; rowIdx < input.rowKeySet().size(); rowIdx++) {
			...
			for (int colIdx = 1; colIdx < input.columnKeySet().size(); colIdx++) {
				...
				csvRow += StringEscapeUtils.escapeCsv(cellValue) + ",";
			}
	...
	public TransformationService getReverseTransformer() {
		return new ReadCSVToTableQuoteOnlyIfNeeded();
	}
}
