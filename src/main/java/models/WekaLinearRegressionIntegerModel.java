...
public class WekaLinearRegressionIntegerModel extends Model{
	private Classifier nativeModel;

	public WekaLinearRegressionIntegerModel(Classifier nativeModel) {
	...
	public String predictForRow(List<String> inputRow) {
    	...
    	for( int colKey=0; colKey < inputRow.size(); colKey++){
    		try{
    			vals[colKey] = Integer.parseInt(inputRow.get(colKey));
    		}
	    ...
	    try {
			return ""+Math.round(nativeModel.classifyInstance(new Instance(1.0, vals)));
		} catch (Exception e) {
			e.printStackTrace();
			return "";
		}
