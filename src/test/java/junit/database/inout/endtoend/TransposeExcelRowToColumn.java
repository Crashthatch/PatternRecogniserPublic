...
public class TransposeExcelRowToColumn {
	...
	{
		AttRelationshipGraph tree = RelationshipFinderInputOutput.processAndGetBestTree("testdata/excel7NumbersAsRow.txt", "testdata/excel7NumbersAsColumn.txt");
		...
		//Try to predict for 20 single-digit numbers.
		{
			...
			String expectedValue = FileUtils.readFileToString( new File("testdata/excel20NumbersAsColumn.txt"));
		...
		//Test data is "different in some respect" (numbers have >1 digit), but splitOnTabs -> recombineWith\n should still work.
		{
			...
			assertTrue( predictions.size() > 0);
			String expectedValue = FileUtils.readFileToString( new File("testdata/excel7MultiDigitNumbersAsColumnWith0s.txt"));
			boolean foundIncorrectPrediction = false;
			for( String prediction : predictions ){
				if( prediction.equals(expectedValue) ){
					System.out.println("Correct Prediction!");
